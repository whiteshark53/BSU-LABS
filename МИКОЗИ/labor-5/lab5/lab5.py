...
def verify(
    ...
    if not (0 <= open_key < p):
        raise ValueError()
    m = int(sha256(bytes(message, encoding="utf-8")).hexdigest(), 16)
    if (pow(e, r, p) * pow(r, s, p)) % p == pow(g, m, p):
        return True
    return False
...
def report():
    with open("report.txt", "w") as f:
        q = 172653929034048084950674429899193619418057473778703331247504102312524336679143
        signature_params, open_key, personal_key = gen(q)
        f.write(f"Gen: {signature_params, open_key, personal_key} \n")
        message = "I, Rakonjac Daniela, love MiKOZI"
