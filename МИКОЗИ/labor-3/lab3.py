...
class RSLOS:
    def __init__(self, a: np.ndarray, c: np.ndarray):
        assert a.shape == c.shape, f'Ожидали, что a и c будут иметь одинаковую форму, но получили: а: {a.shape}, c: {c.shape}'
...
class GepheGenerator:
    def __init__(self, rslos_1: RSLOS, rslos_2: RSLOS, rslos_3: RSLOS):
...
a_1 = np.array([int(i) for i in "00110"])
c_1 = np.array([int(i) for i in "01111"])
a_2 = np.array([int(i) for i in "0011011"])
c_2 = np.array([int(i) for i in "1011011"])
a_3 = np.array([int(i) for i in "11001101"])
c_3 = np.array([int(i) for i in "10010111"])
